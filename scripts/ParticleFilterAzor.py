...
def initParticles(initPoint):
    particles = [];
    for i in range(totalParticles):
        particle = Point(initPoint.x + random.uniform(-10, 10), initPoint.y + random.uniform(-10, 10), initPoint.z)  
...
def assignProbabilitiesToParticles(particles, usblPoint):
    probabilities = [];
    for p in particles:
        distance = math.sqrt( (p.x - usblPoint.x)**2 + (p.y - usblPoint.y)**2)    
        distance = 1. /(usblStandardDev * math.sqrt( 2 * math.pi)) * math.e ** ( - (-distance)**2/(2*usblStandardDev**2))
...
particles = initParticles(Point(2,2,3));
...
def runParticleFilter():
    ...
    pub = rospy.Publisher('/uwsim/PF_AZOR', Point, queue_size=10)
    rospy.init_node('PF_AZOR', anonymous=True)
    ...
    while not rospy.is_shutdown():
        ...
        if(top and usbl and usbl.distance.distance and usbl.bearing.bearing):
            ...
            pointFromUSBL = getPointFromUSBLReading(topPoint, usbl.bearing.bearing, usbl.distance.distance, particles[0].z)
        ...
        if(dvl and imu):
            ...
            if (pointFromUSBL):
                ...
                probabilities = assignProbabilitiesToParticles(particles, pointFromUSBL);
                
                if (sum(probabilities) == 0):
                    particles = initParticles(pointFromUSBL);
                    probabilities = numpy.ones(totalParticles)/totalParticles;                      
